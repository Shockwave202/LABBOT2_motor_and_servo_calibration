...
int eeAddress1, eeAddress2;

#define MOTOR_SETTING_MODE 0
#define SERVO_SETTING_MODE 1
#define RUNNING_MODE 2
int pgm_mode;
bool mode_switching;
//There exists 2 setting modes,thus,an identification bit is
//required to distinguish these 2 modes.
bool setting_mode_identifier;
...
void setup()
{
  ...
  //Output:
  //LED:
  pinMode(A1_BOARD, OUTPUT); //Attach LED1 to A1 pin
  digitalWrite(A1_BOARD, LOW);
  pinMode(A2_BOARD, OUTPUT); //Attach LED2 to A2 pin
...
{                          // run over and over
  ...
  if (pgm_mode == RUNNING_MODE)
  {
    ...
    if (mode_switching)
    {
      ...
      //Set servo2 to 90 degrees.
      servoAddIn[1].expectedAngle = 90;
    ...
    if (enterSettingModePrompt)
    {
      ...
      if (millis() - durationCount > pauseBetweenNotes)
      {
             ...
             //shutdown the tone and exit.
        {
          ...
          if (setting_mode_identifier == MOTOR_SETTING_MODE)
            pgm_mode = MOTOR_SETTING_MODE;
          else //setting_mode_identifier=SERVO_SETTING_MODE
            pgm_mode = SERVO_SETTING_MODE;
          mode_switching = 1;
        }
      }
    }
      ...
      if (rfReading != rfDataCode)
      {
        ...
        else
        {
          ...
          if (!beep_on)
          {
            ...
            case 5: //KEY1
                    //Forward:
              motor1.set(CLKWISE, motor1_forward_speed);
              motor2.set(COUNTER_CLKWISE , motor2_forward_speed);
              digitalWrite(A1_BOARD, HIGH);
              digitalWrite(A2_BOARD, HIGH);
              break;
            case 7: //KEY2
                    //Backward:
              motor1.set(COUNTER_CLKWISE , motor1_backward_speed);
              motor2.set(CLKWISE, motor2_backward_speed);
              digitalWrite(A1_BOARD, HIGH);
              digitalWrite(A2_BOARD, HIGH);
              break;
            case 8: //KEY3
            ...
            case 90: //KEY_A+KEY1
              ...
              mode = FORWARD;
              setting_mode_identifier = MOTOR_SETTING_MODE;
              break;
            case 122: //KEY_A+KEY2
              //Prompt the user that the program is switching to the setting mode.
              if (!enterSettingModePrompt)
            ...
            case 57:  //KEY_B+KEY6
              ...
              setting_mode_identifier = SERVO_SETTING_MODE;
              break;
            }
          }
        }
  ...
  /*--------------MOTOR_SETTING_MODE-----------------*/
  else if (pgm_mode == MOTOR_SETTING_MODE)
  {
    //Entrance:
    if (mode_switching)
    {
  ...
  /*----------------------SERVO_SETTING_MODE-----------------------*/
  else if (pgm_mode == SERVO_SETTING_MODE)
  {
    //Entrance:
    if (mode_switching)
    {
